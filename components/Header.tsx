...
const TRI_COLORS = ['#3B82F6', '#F97316', '#22C55E'] as const
    ...
    <header className='fixed top-0 left-0 right-0 z-50 h-14 bg-white shadow-sm'>
      <div className='mx-auto flex h-full max-w-7xl items-center justify-between px-6 md:px-16'>
        ...
        <Link href='/athlete' className='flex items-center gap-3 group'>
          ...
          <span className='font-display text-sm font-bold uppercase tracking-[0.25em] text-text-primary'>
            My Triathlon
          </span>
        </Link>
          ...
          {NAV_LINKS.map(({ label, href }) => {
            ...
            return (
              ...
              >
                {label}
                {isActive && (
                  <span className='absolute -bottom-[18px] left-0 right-0 h-[2px] bg-text-primary' />
                )}
              </Link>
            )
          })}
